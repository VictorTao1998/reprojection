    ...
    for iteration, data in enumerate(tqdm(val_loader)):
        ...
        if args.onreal:
            ...
            img_R = F.interpolate(img_R, (540, 960), mode='bilinear',
                             recompute_scale_factor=False, align_corners=False)
            img_L_sim = data['img_L_sim'].cuda()    # [bs, 1, H, W]
            img_R_sim = data['img_R_sim'].cuda()  # [bs, 1, H, W]
            with torch.no_grad():
            ...
            with torch.no_grad():
                img_real = data['img_L_real'].cuda()  # [bs, 1, H, W]
                img_L_transformed, img_R_transformed, _, _ = transformer_model(img_L, img_R, img_real, img_real)
